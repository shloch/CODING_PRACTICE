...
# EXERCISE --> https://www.codewars.com/kata/equal-sides-of-an-array/train/ruby
...
# of the integers to the right of N. If there is no
...
# ({1}) and the sum of the right side of the index
...
def find_even_index(arr)
  ...
  arr.each_with_index do |_, i|
    if i.zero? # stating index 0
      left_total = 0
      right_total = arr[1..arr.size - 1].sum
    elsif i == arr.size - 1 # last index
      right_total = 0
      left_total = arr[0...arr.size - 1].sum
    else # when index somewhere at middle
      left_total = arr[0..i - 1].sum
      right_total = arr[i + 1..arr.size - 1].sum
    end
    return i if left_total == right_total
  end
