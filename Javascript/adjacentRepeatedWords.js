...
KATA = https://www.codewars.com/kata/5245a9138ca049e9a10007b8
...
As as aS you can see, it's not easy to spot those errors, especially if
 words differ in case, like "as" at the beginning of this sentence.
...
Write a function that counts the number of sections repeating the same
 word (case insensitive). The occurence of two or more equal words next
  after each other counts as one.
...
function countAdjacentPairs(searchString) {
    ...
    let lowercase_arr = arr.map( word => word.toLowerCase())
    let count = 0
    
    let i = 1
    while(i < lowercase_arr.length) {
      if(lowercase_arr[i] === lowercase_arr[i-1]) {
        count = count + 1
        i = i + 1
        while(lowercase_arr[i] === lowercase_arr[i-1]) {
          i = i + 1
        }
